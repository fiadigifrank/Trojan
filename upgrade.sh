...
getOldVer=$(/usr/bin/trojan-go/trojan-go -version |grep Trojan-Go |head -n 1|cut -d ' ' -f 2)
...
cp -f /tmp/trojan/trojan-go /tmp/trojan/geo* /usr/bin/trojan-go && echo -e "${green}新版本覆盖安装完毕${plain}"
getNewVer=$(/usr/bin/trojan-go/trojan-go -version |grep Trojan-Go |head -n 1|cut -d ' ' -f 2)
